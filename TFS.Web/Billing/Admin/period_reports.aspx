...
<asp:Content ID="headScript" runat="server" ContentPlaceHolderID="MoreStylesheets">
    <script>
        function ValidateTimesheetStart() {
            var drpDwnUserTimesheetStart = $('#<%= drpDwnActiveUsers.ClientID %>');
            var drpDwndrpDwnAccounts = $('#<%= drpDwnAccounts.ClientID %>');

            if (!drpDwnUserTimesheetStart.val()) {
                alert("Please select Personnel");
                return false;
            }

            if (!drpDwndrpDwnAccounts.val()) {
                alert("Please select Account");
                return false;
            }

            var userFullName = drpDwnUserTimesheetStart.find('option:selected').text();
            var accountName = drpDwndrpDwnAccounts.find('option:selected').text();

            return confirm("Are you sure you want to start a Timesheet for: " + userFullName + ", for Account: " + accountName + " ?");
        }
    </script>
</asp:Content>
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <asp:ObjectDataSource ID="ActiveUsersDataSource" runat="server"
        TypeName="TFS.Intranet.Data.Billing.UserController"
        SelectMethod="FetchAllActiveOrderedByLastName">
    </asp:ObjectDataSource>
    ...
    <asp:UpdatePanel ID="AllTimeSheetsUpdate" runat="server">
        <ContentTemplate>
                ...
                <HeaderTemplate>
                    <table>
                        <tr>
                            ...
                            <th style="width: 250px;">Billing Account Name</th>
                        </tr>
                    </table>
                </HeaderTemplate>
                <ItemTemplate>
                    <asp:Panel CssClass="popupMenu" ID="PopupMenu" runat="server">
                        <div style="border: 1px outset white; padding: 5px;">
                            <div>
                                <a href='<%# "../Reports/timecardreport.aspx?username="+Eval("username")+"&id="+Eval("timesheetid")+"&month="+Eval("month")+"&year="+Eval("year") %>'>View Report</a>
                            </div>
                            <div>
                                <a href='<%# "../timecard.aspx?id="+Eval("timesheetid").ToString()+"&username="+Eval("username").ToString()+"&month="+Eval("month")+"&year="+Eval("year") %>'>Edit Timesheet Data</a>
                            </div>
                            <div>
                                <a href='<%# "timecard_modify.aspx?id="+Eval("timesheetid").ToString()+"&username="+Eval("username").ToString()+"&month="+Eval("month")+"&year="+Eval("year")+"&periodid="+Request.Params["id"] %>'>Merge Timesheet Data</a>
                            </div>
                            <div>
                                ...
                                <ajax:ConfirmButtonExtender ID="ComfirmDelete" runat="server" TargetControlID="DeleteTimesheetButton"
                                    ConfirmText="Are you sure you want to delete this timesheet?">
    ...
    <a class="topOfPage" href="#top" title="Go to the top of this page">^ TOP</a>
    <h1 id="H1">Start New Employee Timesheets</h1>
    <br />
    <% if (IsActivePeriod)
       { %>
    <asp:DropDownList runat="server" ID="drpDwnActiveUsers" DataSourceID="ActiveUsersDataSource"
        DataTextField="FullName" DataValueField="username" AppendDataBoundItems="True">
        <asp:ListItem Value="" Text="Select Personnel"></asp:ListItem>
    </asp:DropDownList>
    &nbsp;
    &nbsp;
    
      <asp:DropDownList runat="server" ID="drpDwnAccounts" DataSourceID="PendingTimesheetInfoData"
        DataTextField="AccountName" DataValueField="AccountId" AppendDataBoundItems="True">
        <asp:ListItem Value="" Text="Select Account"></asp:ListItem>
    </asp:DropDownList>
    
    <br />
    <br />
    
    <asp:LinkButton runat="server" ID="lnkBtnStartUserTimesheet" OnClientClick="return ValidateTimesheetStart()" OnClick="lnkBtnStartUserTimesheet_OnClick" Text="Start Timesheet for Personnel"></asp:LinkButton>
    <% } %>
    <%
       else
       { %>
    <span style="color:red">Not an active period</span>
    <% } %>
</asp:Content>
